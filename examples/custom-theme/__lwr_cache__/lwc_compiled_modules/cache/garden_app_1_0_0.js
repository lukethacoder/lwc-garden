...
class App extends LightningElement {
  constructor(...args) {
     ...
     */
    this.moduleLwc = void 0;
    /**
     * Metadata of the currently selected module
     */
    this.moduleMetadata = void 0;
    this.moduleLwcProps = {};
    this.handleClickModule = async event => {
    ...
    this.handleSetModuleByName = async moduleName => {
      this.moduleLwcProps = {};
      const moduleMetadata = this.MODULES.find(module => module.name === moduleName);
      if (!moduleMetadata) {
        this.moduleLwc = undefined;
        return;
      }
  ...
  renderedCallback() {
    if (this.isFirstRender) {
      ...
      const resizePropsSliderEl = this.template.querySelector('.resize_props_slider_el');
      const resizePropsTargetEl = this.template.querySelector('.resize_props_target_el');
...
_registerDecorators(App, {
  ...
  fields: ["MODULES", "isFirstRender", "handleClickModule", "handlePropsChange", "handlePropsClear", "handleSetModuleByName"]
});
