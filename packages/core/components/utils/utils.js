 ...
 * @param {HTMLElement | HTMLIFrameElement} iframe
 * @returns {Boolean}
 */
 ...
 * @param {(node) => void} callback Function to call on each node element.
 */
export function traverseActiveTreeRecursively(node, callback) {
  if (!node) {
    return
  }
